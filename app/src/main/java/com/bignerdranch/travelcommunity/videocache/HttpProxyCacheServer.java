...
import com.bignerdranch.travelcommunity.videocache.file.TotalCountLruDiskUsage;
import com.bignerdranch.travelcommunity.videocache.file.TotalSizeLruDiskUsage;
import com.bignerdranch.travelcommunity.videocache.headers.EmptyHeadersInjector;
...
import com.bignerdranch.travelcommunity.videocache.sourcestorage.SourceInfoStorageFactory;
...
import static com.bignerdranch.travelcommunity.videocache.Preconditions.checkAllNotNull;
...
public class HttpProxyCacheServer {
    ...
    private final com.bignerdranch.travelcommunity.videocache.Pinger pinger;

    public HttpProxyCacheServer(Context context) {
        this(new Builder(context).buildConfig());
    }

    private HttpProxyCacheServer(com.bignerdranch.travelcommunity.videocache.Config config) {
        ...
        try {
            ...
            this.pinger = new com.bignerdranch.travelcommunity.videocache.Pinger(PROXY_HOST, port);
    ...
    public void registerCacheListener(com.bignerdranch.travelcommunity.videocache.CacheListener cacheListener, String url) {
        checkAllNotNull(cacheListener, url);
        synchronized (clientsLock) {
            try {
                getClients(url).registerCacheListener(cacheListener);
            } catch (com.bignerdranch.travelcommunity.videocache.ProxyCacheException e) {
                LOG.warn("Error registering cache listener", e);
            }
        }
    }

    public void unregisterCacheListener(com.bignerdranch.travelcommunity.videocache.CacheListener cacheListener, String url) {
        checkAllNotNull(cacheListener, url);
        synchronized (clientsLock) {
            try {
   ...
   public void tryLoad(String url){
       HttpProxyCacheServerClients client =  clientsMap.get(url);
       if(client != null){
           client.tryLoad();
           LogUtil.INSTANCE.ee("url+" + client);
       }
   }

    public void canLoad(String url){
      HttpProxyCacheServerClients client =  clientsMap.get(url);

       if(client != null) client.canLoad();
    }
    ...
    public void shutdown() {
        ...
        try {
            ...
            onError(new com.bignerdranch.travelcommunity.videocache.ProxyCacheException("Error shutting down proxy server", e));
        }
    }
    ...
    private String appendToProxyUrl(String url) {
        return String.format(Locale.US, "http://%s:%d/%s", PROXY_HOST, port, com.bignerdranch.travelcommunity.videocache.ProxyCacheUtils.encode(url));
    }
    ...
    private void waitForRequest() {
        try {
            ...
            onError(new com.bignerdranch.travelcommunity.videocache.ProxyCacheException("Error during waiting connection", e));
        }
    }
    ...
    private void processSocket(Socket socket) {
        try {
            com.bignerdranch.travelcommunity.videocache.GetRequest request = com.bignerdranch.travelcommunity.videocache.GetRequest.read(socket.getInputStream());
            LOG.debug("Request to cache proxy:" + request);
           // LogUtil.INSTANCE.ee("request" + request + "socket"+socket);
            String url = ProxyCacheUtils.decode(request.uri);
            if (pinger.isPingRequest(url)) {
               pinger.responseToPing(socket);
               LogUtil.INSTANCE.e("ping..." + socket);
           } else {
                HttpProxyCacheServerClients clients = getClients(url);
                LogUtil.INSTANCE.ee("执行ProcessSocket" + url);
                clients.processRequest(request, socket);
                LogUtil.INSTANCE.ee("结束ProcessSocket" + socket);
            }
        ...
        } catch (com.bignerdranch.travelcommunity.videocache.ProxyCacheException | IOException e) {
            onError(new com.bignerdranch.travelcommunity.videocache.ProxyCacheException("Error processing request", e));
        } finally {
            releaseSocket(socket);
            LogUtil.INSTANCE.ee("releaseSocket");
    ...
    private HttpProxyCacheServerClients getClients(String url) throws com.bignerdranch.travelcommunity.videocache.ProxyCacheException {
        synchronized (clientsLock) {
    ...
    private void closeSocketInput(Socket socket) {
        try {
            ...
            onError(new com.bignerdranch.travelcommunity.videocache.ProxyCacheException("Error closing socket input stream", e));
        }
    }
    ...
    private final class SocketProcessorRunnable implements Runnable {
        ...
        @Override
        public void run() {
            LogUtil.INSTANCE.e("processSocket");
    ...
    public static final class Builder {
        ...
        public Builder(Context context) {
            ...
            this.cacheRoot = com.bignerdranch.travelcommunity.videocache.StorageUtils.getIndividualCacheDirectory(context);
        ...
        public HttpProxyCacheServer build() {
            com.bignerdranch.travelcommunity.videocache.Config config = buildConfig();
            return new HttpProxyCacheServer(config);
        }

        private com.bignerdranch.travelcommunity.videocache.Config buildConfig() {
            return new com.bignerdranch.travelcommunity.videocache.Config(cacheRoot, fileNameGenerator, diskUsage, sourceInfoStorage, headerInjector);
        }
